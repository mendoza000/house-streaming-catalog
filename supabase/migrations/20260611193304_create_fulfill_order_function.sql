-- Entrega atómica de una orden: asigna pantallas libres a las cuentas del
-- servicio, registra clients + sales, y devuelve las credenciales entregadas.
--
-- Garantías:
--  * Atómica: corre en una transacción. Si falta stock -> OUT_OF_STOCK y rollback
--    total (nunca entrega parcial).
--  * Idempotente: si la orden ya tiene ventas, devuelve lo entregado sin reasignar.
--  * Concurrencia: FOR UPDATE SKIP LOCKED bloquea la cuenta candidata; dos órdenes
--    simultáneas no pueden tomar la misma pantalla (la perdedora salta la cuenta).
