...
/**
 * Updates a draft order to pending status (when user proceeds with payment)
 * @param orderId - The order ID
 * @returns Promise with the updated order
 */
export async function updateOrderToPending(
	orderId: number,
): Promise<{ data: Order | null; error: Error | null }> {
	try {
		const updateData: OrderUpdate = {
		...
		console.error("Unexpected error updating order to pending:", error)
		return {
			data: null,
			error:
				error instanceof Error
					? error
					: new Error("Failed to update order to pending"),
		}
	}
}
...
export async function updateOrderStatus(
	...
	try {
		...
		return {
			data: null,
			error:
				error instanceof Error ? error : new Error("Failed to update order"),
		}
	}
}
